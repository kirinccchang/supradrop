...
SupraDrop 在您的瀏覽器中直接讀取 `.docx` 文件，掃描每一個腳注，並針對六大類別的 **19 條《藍皮書》規則**進行檢查：*supra* 的使用、*Id.* 的使用、引用信號詞括號說明、頁碼引用（pincite）、網址封存，以及 *hereinafter* 的一致性。
...
本儲存庫附有 [`SupraDrop_TestFixture.docx`](SupraDrop_TestFixture.docx)，這是一份特意設置了引用格式錯誤的 Word 文件，涵蓋全部 19 條規則的範例。您可以將它上傳至 [lawreview.tools/supradrop](https://lawreview.tools/supradrop/)，立即看到所有檢查功能的運作效果。
